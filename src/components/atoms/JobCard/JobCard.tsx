...
const JobCard: React.FC<JobCardProps> = memo(({ title, department, location, employmentType, href }) => {
  return (
    <div className="size-full">
      ...
      >
        <div className="flex flex-col h-full gap-space-12 p-space-10">
          <div className="flex flex-col h-full gap-space-12">
            <div className="flex flex-wrap-reverse items-center gap-space-08 sm:gap-space-10">
              ...
              {employmentType
                && (
                  <div className="text-primary ml-auto h-fit text-size-4xs border border-border-color bg-container-gradient rounded-sm px-space-05 py-space-02">
                    {employmentType}
                  </div>
                )}
            </div>
