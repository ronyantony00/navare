...
```shell
git clone git@bitbucket.org:innogenio-repo/innogenio-website-frontend.git
cd innogenio-website-frontend
npm install
```
...
This project is configured for AWS Amplify Hosting with CloudFront CDN and automatic SSL certificates.
...
5. The `amplify.yml` file will be automatically detected
...
- ✅ Built-in monitoring
- ✅ Cost-effective ($5-20/month after free tier)

#### Local Production Build

For testing purposes, you can generate a production build locally:
...
**Note:** During the build process, database migrations are automatically executed, so there's no need to run them manually. However, you must define `DATABASE_URL` in your environment variables.
